...
async function loadPageStyles(htmlPath) {
	...
	const linkedStyles = await Promise.all(
		stylesheetUrls.map(async (stylesheetUrl) => {
			let pathname = decodeURIComponent(stylesheetUrl.split(/[?#]/, 1)[0]);
			if (
				/^(?:[a-z]+:)?\/\//i.test(pathname) ||
			...
			// Astro emits assets at dist/_astro even when a deployment base prefixes
			// their public URLs (for example /repository/_astro on GitHub Pages).
			const astroAssetIndex = pathname.indexOf("/_astro/");
			if (astroAssetIndex >= 0) {
				pathname = pathname.slice(astroAssetIndex);
			}
...
const pages = [
	...
	},
	{
		...
		requiredMarkup: [["关于这个出版室", "publication introduction"]],
		requiredRules: [
			["--publication-surface", "publication surface token"],
			[".publication-page", "publication page styles"],
		],
	},
];
