...
describe("Global style loading regressions", () => {
	...
	it("loads Markdown styles from the Markdown wrapper", () => {
		...
		assert.match(globalStyleCheckSource, /关于这个出版室/);
		assert.match(globalStyleCheckSource, /--publication-surface/);
		assert.match(globalStyleCheckSource, /\.publication-page/);
	});
});
...
describe("Page scaling regressions", () => {
	it("keeps root-font scaling disabled in the fixed publication layout", () => {
		assert.match(
			siteConfigSource,
		...
		assert.doesNotMatch(
			headTagsSource,
			/pageScaling|adjustPageScale/,
			"the publication bootstrap must not restore legacy root scaling",
		);
	});
});
...
describe("Page layout variant regressions", () => {
	...
	it("keeps the wide-screen post layout fixed and pre-painted", () => {
		assert.match(
			siteConfigSource,
			/ultrawidePostLayout:\s*\{[\s\S]*?enable:\s*true[\s\S]*?allowSwitch:\s*false/,
			"the wide-screen layout stays enabled without a removed settings toggle",
		);
		assert.match(
			layoutSource,
		...
		assert.doesNotMatch(
			...
			"the publication bootstrap must ignore a stale visitor preference",
		);
