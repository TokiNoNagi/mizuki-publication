...
describe("Markdown AST enhancements", () => {
	...
	it("turns standalone wiki links into covered cards and inline links", async () => {
			...
			children: [
				...
				},
				{
					...
					children: [
						{
							type: "text",
							value: "See [[start-here|the start page]].",
						},
					],
				},
			],
		...
		await remarkWikiLink()(tree, {
			path: fileURLToPath(
				new URL("./fixtures/content-pipeline-fixture.mdx", import.meta.url),
			),
		});
		...
		assert.equal(
			tree.children[0].children.some((child) =>
				child.data?.hProperties?.class?.includes?.("wlc-cover"),
			),
			false,
		);
