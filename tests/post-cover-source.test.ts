...
const fixturePath = path.join(
	repositoryRoot,
	"tests/fixtures/post-cover/fixture.md",
);
...
describe("shared post cover resolution", () => {
	...
	it("resolves an existing post-relative cover", async () => {
		const cover = await resolvePostCoverSource(
			{ image: "./cover-placeholder.txt" },
			{ contentFilePath: fixturePath, basePath: "tests/fixtures/post-cover" },
		);
		assert.equal(cover.kind, "local");
		assert.equal(cover.basePath, "tests/fixtures/post-cover");
		assert.equal(
			cover.absolutePath,
			path.join(path.dirname(fixturePath), "cover-placeholder.txt"),
		);
	});
